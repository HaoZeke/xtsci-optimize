...
The curvature condition demands |phi'(a)| <= c2 |phi'(0)| with
...
The width therefore decays geometrically (``Rgmin.widthBound`` is the
envelope, with ``widthBound_le_start`` and ``widthBound_nonneg`` pinning
its shape), and the evaluation budget of the zoom is logarithmic in the

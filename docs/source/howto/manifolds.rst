...
.. table::
    ...
    +-------------------+---------------------------------------+-------------------------------------------+
    | ``Spd``           | row-major ``n x n``, length ``n^2``   | ``symm(X + U + (1/2) U X^{-1} U)``        |
    +-------------------+---------------------------------------+-------------------------------------------+
...
``Sphere``, ``So3``, ``Stiefel``, ``Se3``, and ``Spd`` are matrix-manifold
embeddings. ``So3`` rejects any length other than 9. ``Se3`` rejects
any length other than 12. ``Spd`` rejects a length that is not a
positive perfect square. They do not pack or prefix-interpret a
3N cluster.
...
- ``Spd`` is row-major ``n^2``, manopt ``sympositivedefinitefactory``.
  The tangent projection is symmetrization. The Riemannian
  gradient is ``X * symm(egrad) * X``. A 3N cluster is not this
  packing.

...
9. ~Se3~ rejects any length other than 12. ~Hyperbolic~ is the
Lorentz hyperboloid (manopt ~hyperbolicfactory~, \(m = 1\)):
length \(n\) (\(n \ge 2\)), Minkowski square \(-1\). They do not
pack or prefix-interpret a 3N cluster.
...
- ~Hyperbolic~ is a length-\(n\) Minkowski vector
  (\(-x_0^2 + \|x_{\mathrm{sp}}\|_2^2 = -1\), \(n \ge 2\)).
  Pack with ~pack~ / ~unpack~ (time-like then spatial). It is

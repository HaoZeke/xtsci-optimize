...
TEST_CASE("BisectionSearch Strategy Test with Different Initial Bounds", "[BisectionSearch]") {
    xts::optimize::trial_functions::QuadraticFunction<double> quadratic;
    ...
    SECTION("Using WeakWolfeCondition with alpha_min = 0.2, alpha_max = 0.8") {
        xts::optimize::linesearch::search_strategy::BisectionSearch<double> bisection(0.2, 0.8);
        xts::optimize::linesearch::conditions::WeakWolfeCondition<double> condition;
        double alpha = bisection.search(quadratic, { x, direction }, condition);
        REQUIRE(condition(alpha, quadratic, { x, direction }) == true);
    }
}
TEST_CASE("BisectionSearch Strategy Test with Different Tolerance and Max Iterations", "[BisectionSearch]") {
    xts::optimize::trial_functions::QuadraticFunction<double> quadratic;
    ...
    SECTION("Using WeakWolfeCondition with tol = 1e-3 and max_iterations = 5") {
        ...
        xts::optimize::linesearch::search_strategy::BisectionSearch<double> bisection(0.0, 1.0, control);
        xts::optimize::linesearch::conditions::WeakWolfeCondition<double> condition;
        double alpha = bisection.search(quadratic, { x, direction }, condition);

        REQUIRE(condition(alpha, quadratic, { x, direction }) == true);
        REQUIRE(quadratic(x + alpha * direction) < quadratic(x));  // Test if the function value has decreased
    }
...
TEST_CASE("BisectionSearch Convergence Test", "[BisectionSearch]") {
    ...
    SECTION("Using WeakWolfeCondition") {
        ...
        // Test if the function value at the new point is less than the value at the starting point

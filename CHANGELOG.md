...
- Sella `TrustRegion` restricted step (`cons(s) = ||s||`, target
  `delta`) on the quasi-Newton family `s(α)`. Distinct from
  IRCTrustRegion (`||(s+d1)*sqrt(m)||`) and from the Powell dogleg.
